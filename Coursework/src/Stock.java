	...
	public boolean addNewProduct(String[] details) {
		...
		try {
			...
			if (category.equalsIgnoreCase("board game")) {
				...
				productList.add(new BoardGame(id, ProductCategory.BOARDGAME, type, name, price, stock, cost, maxPlayers));
			} else if (category.equalsIgnoreCase("accessory")) {
				productList.add(new Accessory(id, ProductCategory.ACCESSORY, type, name, price, stock, cost, additional));
			}
	...
	public ArrayList<Product> searchByCompatibility(String compQuery) {
	    ...
	    for (Product p : productList) {
	        if (p instanceof Accessory) {
	            ...
	            // Exact match for short queries, partial for long queries
